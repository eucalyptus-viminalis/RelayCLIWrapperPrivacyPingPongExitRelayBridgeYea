...
To make the `relay` command available globally:
...
```bash
relay wallet derive
relay wallet derive --start-index 0 --count 5
```

To make the CLI use a different derived address index:

```bash
relay config set-mnemonic-index 1
relay config show
```
...
```bash
relay config set-private-key-env WALLET_PK
...
```bash
relay config set-mnemonic-env WALLET_SEED
...
```bash
relay config unset-private-key-env
relay config unset-mnemonic-env
relay config unset-mnemonic-index
```
...
```bash
relay config set-proxy socks5h://127.0.0.1:9150
```
...
```bash
relay --json bridge --from base --to optimism --token usdc --amount 25 --quote-only
relay --json status 0xrequestid
relay --json wallet derive
relay --json token search usdc
```
...
```bash
relay bridge --from base --to optimism --token usdc --amount 25 --quote-only
```

Use your exact origin-token balance instead of typing an amount:

```bash
relay bridge arbitrum:weth ethereum:eth --max --quote-only --wallet 0xYourWalletAddress
relay bridge arbitrum:weth ethereum:eth --max --use-permit
```

The `bridge` command also accepts a compact shorthand:

```bash
relay bridge arbitrum:weth ethereum:eth 0.01 --quote-only
```

Preview a quote with a specific wallet context, without needing a configured private key:

```bash
relay bridge --from arbitrum --to ethereum --token weth --amount 0.01 --quote-only --wallet 0xYourWalletAddress
```

Bridge and execute the steps with your configured signer:

```bash
relay bridge --from ethereum --to base --token eth --amount 0.05
```

When you want Relay's permit-based route for the most gas-efficient ERC-20 path, add:

```bash
relay bridge arbitrum:weth ethereum:eth 0.01 --use-permit
```

When you want the CLI to refuse any route that still needs an onchain wallet transaction, add strict gasless mode:

```bash
relay bridge arbitrum:weth ethereum:eth 0.01 --quote-only --strict-gasless
relay bridge arbitrum:weth ethereum:eth 0.01 --use-permit --strict-gasless
```
...
```bash
relay proxy check
relay proxy check --wallet 0xYourWalletAddress
```
...
```bash
relay api list
relay api describe POST /quote/v2
```

Search tokens by name or symbol:

```bash
relay token search "wrapped ether" --chain arbitrum
relay token search weth --limit 5
```

Call any Relay endpoint:

```bash
relay api call GET /chains
relay api call POST /quote/v2 --body '{"user":"0x...","originChainId":8453,"destinationChainId":10,"originCurrency":"0x0000000000000000000000000000000000000000","destinationCurrency":"0x0000000000000000000000000000000000000000","amount":"100000000000000000","tradeType":"EXACT_INPUT"}'
```
